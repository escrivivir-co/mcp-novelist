...
export class SocketServer {
  ...
  onConnection(namespace: string, socket: Socket) {
    this.log(
      'New connection at',
      namespace + '. Socket: ' + this.socketName(socket) + ': ' + socket.id
    );

    if (this.autoBroadcast) {
      socket.on('CLIENT_REGISTER', (data, ...args) => {
        this.onClientRegister(namespace, socket, data);
      });
      socket.on('CLIENT_SUSCRIBE', (data, ...args) => {
        this.onClientSuscribe(namespace, socket, data);
      });
      socket.on('ROOM_MESSAGE', (data, ...args) => {
  ...
  onClientSuscribe(namespace: string, socket: Socket, args: SuscriptionDetails) {
    const message =
      namespace + '.onClientSuscribe.' + this.sockets.get(socket.id)?.name ||
      'El socket no se ha registrado: ' + socket.id;
    if (args.out) {
      socket.leave(args.room);
      this.log('LEAVE:> ' + message, args);
    } else {
      socket.join(args.room);
      this.log('JOIN:> ' + message, args);
    }
  }
      ...
      if (master) {
        this.log(
          namespace +
            '/' +
            this.socketName(socket) +
            ':> Resolving GETTER... master found, emit [' +
            args.event +
            '] TO!',
